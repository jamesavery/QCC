    ...
    match(rule):
        case ['TYPE','lval']: # Zero initialization
            ...
            if is_resolved: # Do we know the address of the lval?
                ...
                #print(f"scope = {scope}, l = {l}, size = {size}")
                if size is None:
                    value_env[scope][name] = 0
                else:
                    value_env[scope][name] = [0]*size
                #print(f"initialized {l}, env = {env}")
                return (result,True)
            else:
                return (result,False)
            
        case ['TYPE','ID',_]: # Scalar initialization; lval always resolved.
            ...
            if v_static:
                scope = lookup_scope(l, value_env) # In which scope is l defined?
                value_env[scope][l] = v
                return (result,True)
            else:
                return (result,False)
        ...
        case ['TYPE','ID',_,'exps']: # Array declaration with initialization 
            [type,name,exp_size,exps] = d.children
            l = f"{name}"
            (size,size_static)   =  PE_exp(exp_size,value_env)
            ...
            if size_static and all([static for (v,static) in values]):
                scope = lookup_scope(l, value_env) # In which scope is l defined?
                value_env[scope][l] = [v for (v,static) in values]
                return (result,True)
            else:
                return (result,False)
        case _:
    ...
    try:
        match(rule):
            case ['lval', 'EQ', _]: # Assignment
                ...
                #print(f"['lval', 'EQ', _]: {showcq_lval(lval)} = {showcq_exp(exp)}")                    
                ...
                if lhs_static and rhs_static:
                    scope = lookup_scope(name, value_env) # In which scope is l defined?
                    if index is None:
                        value_env[scope][name] = rhs
                    else:
                        value_env[scope][name][index] = rhs
                    #print(f"resolved assignment, env = {value_env}")  
                    return (skip,True)
                else:
                    #print(f"unresolved assignment, {showcq_lval(lv)} = {showcq_exp(rhs)}")
            ...
            case ['IF', _, 'statement', 'statement']:
                ...
                if c_static:
                    ...
                    #print(f"IF: {c} is not numeric. DON'T PE STATEMENTS, AS THIS WILL CHANGE STATE")
                    # TODO: Copy the environment, and apply the side-effects to the copy. Then replace statements by blocks that apply side-effects as assignments.
                    #s1 = PE_statement(s1, value_env)
                    #s2 = PE_statement(s2, value_env)
                    result.children = [IF, c, s1, s2]
                    return (result,False)
            
            case ['WHILE', _, 'statement']:
                [WHILE, cond, s1] = s.children
                #print(f"WHILE: {showcq_exp(cond)} {showcq_statement(s1)}")
                (c,c_static) = PE_exp(cond, value_env)
                ...
                if c_static: 
                    ...
                    if residual_statements == []:
                        ...
                        return (make_block([],residual_statements,True), False)
                else:
                    (s1,s1_static) = PE_statement(s1, value_env)
            ...
            case ['block']:
                ...
                if residual_declarations == [] and residual_statements == []:
                    return (skip,True)
                else: 
                    return (make_block(residual_declarations,residual_statements,True), False)
